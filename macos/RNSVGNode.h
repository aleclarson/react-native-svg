...
/**
 * RNSVG nodes are implemented as base NSViews. They should be implementation for all basic
 ＊interfaces for all non-defination nodes.
 */

@interface RNSVGNode : NSView
...
/**
 * RNSVGRootView which ownes current RNSVGNode
 */
@property (nonatomic, readonly, weak) RNSVGRootView *rootView;
...
- (void)traverseSubviews:(BOOL (^)(__kindof NSView *node))block;
